...
class HPUCompressedTensorsW8A8Fp8(CompressedTensorsScheme):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # print(f"======================process_weights_after_loading, weight_scale: {layer.weight_scale}")
        # print(f"======================process_weights_after_loading, input_scale: {layer.input_scale}")
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor] = None):
        ...
        print(f"=======================================input_scale: {input_scale.shape}")
...
class HPUCompressedTensorsW8A8Fp8MoEMethod(CompressedTensorsW8A8Fp8MoEMethod):
    ...
    def create_weights(self, *args, **kwargs) -> None:
        print("HPUCompressedTensorsW8A8Fp8MoEMethod..................................create_weights")
        if hpu_ops.is_hpu_gaudi2:
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        print("HPUCompressedTensorsW8A8Fp8MoEMethod..................................process_weights_after_loading")
        print(f"==========layer.w13_weight_scale: {layer.w13_weight_scale}")
        print(f"==========layer.w2_weight_scale: {layer.w2_weight_scale}")
        print(f"==========layer.w13_input_scale: {layer.w13_input_scale}")
        print(f"==========layer.w2_input_scale: {layer.w2_input_scale}")
        ...
        if self.weight_quant.strategy == QuantizationStrategy.TENSOR:
            ...
            w13_weight_scale_channel = torch.empty(
                (layer.local_num_experts, 2 * layer.intermediate_size_per_partition, 1), dtype=torch.float32, device=layer.w13_weight_scale.device)
            w13_weight_scale_channel[:, :layer.intermediate_size_per_partition, 0] = layer.w13_weight_scale[:, :1]
            w13_weight_scale_channel[:, layer.intermediate_size_per_partition:, 0] = layer.w13_weight_scale[:, 1:]
            layer.w13_weight_scale = torch.nn.Parameter(w13_weight_scale_channel,
                                                        requires_grad=False)

            w2_weight_scale_channel = torch.empty(
                (layer.local_num_experts, layer.hidden_size, 1), dtype=torch.float32, device=layer.w13_weight_scale.device)

            w2_weight_scale_channel[:, :, 0] = layer.w2_weight_scale.reshape(-1, 1)
            layer.w2_weight_scale = torch.nn.Parameter(w2_weight_scale_channel,
                                                        requires_grad=False)
